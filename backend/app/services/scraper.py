...
def _scrape_search(page, keywords, location, remote_type, experience_level, max_pages=3) -> list[dict]:
    """Scrape one search query using an already-authenticated page."""
    search_url = _build_search_url(keywords, location, remote_type, experience_level)
    ...
    for page_num in range(max_pages):
        ...
        for i, card in enumerate(cards):
            try:
                ...
                logger.info(f"  [{i+1}] {title} @ {company}")
...
def _run_single_sync(keywords, location, remote_type, experience_level, max_pages=3) -> list[dict]:
    ...
    with sync_playwright() as p:
        ...
        jobs = _scrape_search(page, keywords, location, remote_type, experience_level, max_pages)
        _save_cookies(page)
        browser.close()
...
async def search_linkedin_jobs(
    ...
    return await asyncio.to_thread(
        _run_single_sync, keywords, location, remote_type, experience_level, max_pages
    )
